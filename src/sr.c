...
struct Bbuffer
{
	int rcvstatus;		//denotes the received status for a particular seqnum  on B side
	int bseqnum;
};
...
/*This structure will be used to store the values of the startime, expected timeout and the current seqnum*/
/*also used for maintaining the buffer details on both ends*/


struct Abuffer A_buff[1000];	//this will be used to for maintaining the event data on the A's side
struct Bbuffer B_buff[1000];
...
void revaltimer(int *min, int * locseq)
{
	int k,loc;
	*min=9999999;//intilaizing to a high number

	for(k=base;(k<base+WINSIZE && k < nsimmax);k++)
	{
		if(A_buff[k].ackstatus!=1 && A_buff[k].starttime!=0)
		{
			if(A_buff[k].endtime<*min)
			{
				*min=A_buff[k].endtime;
				*locseq=k;
			}
		}
	}
...
for(i=nextseq;i<(base+WINSIZE) && A_buff[i].pack.acknum!=-11 && i < nsimmax;i++)
{
	if(status==1)
	{
	...
	A_transport++;
	tolayer3(0,A_buff[i].pack);  					//sent the packet from A (ie 0 ) to 3 layer packet which forwards to B
	nextseq++;
	printf("packet:: seqnum:%d acknum:%d payload:%s checksum:%d\n",A_buff[i].pack.seqnum,A_buff[i].pack.acknum,A_buff[i].pack.payload,A_buff[i].pack.checksum);

	A_buff[i].starttime=time;				//time when the timer was started for a particular seqnum
	A_buff[i].endtime=A_buff[i].starttime+TIMEOUT;		//expected timeout for the particular seqnum
...
struct msg message;
{
	int size=sizeof(A_buff[A_application].pack.seqnum)+sizeof(A_buff[A_application].pack.acknum)+sizeof(A_buff[A_application].pack.payload);
	...
	if(nextseq<(base+WINSIZE))
	{
		...
		memcpy(A_buff[A_application].pack.payload,message.data,sizeof(message.data));
		A_buff[A_application].pack.acknum=-1;
		//	calculate the checksum //

		memcpy(buffer,&A_buff[A_application].pack.seqnum,sizeof(A_buff[A_application].pack.seqnum));
		memcpy(buffer+sizeof(A_buff[A_application].pack.seqnum),&A_buff[A_application].pack.acknum,sizeof(A_buff[A_application].pack.acknum));
		memcpy(buffer+sizeof(A_buff[A_application].pack.seqnum)+sizeof(A_buff[A_application].pack.acknum),A_buff[A_application].pack.payload,sizeof(A_buff[A_application].pack.payload));
		A_buff[A_application].pack.checksum=CheckSum((int *)buffer,sizeof(buffer));

		//	send to the other side //
		A_send();

	}
	else		/*if message received beyond the window size then saving into buffer*/
	{
		memcpy(A_buff[A_application].pack.payload,message.data,sizeof(message.data));		//the new message data was copied to the A_applicaiton th location
		A_buff[A_application].pack.acknum=-1;
...
if((base==packet.acknum)&&(A_inputchecksum==packet.checksum)&&(A_buff[base].ackstatus!=1))
{
	A_buff[base].ackstatus=1;//set acknowledgement of the received pkt..not considering the timeout of that seq
	base++;
	stoptimer(0);
	status=1;
	revaltimer(&min, &locseq);
...
else if((base<packet.acknum)&&(A_inputchecksum==packet.checksum))
{
	A_buff[packet.acknum].ackstatus=1;
	revaltimer(&min, &locseq);


	if (min==packet.acknum){
		stoptimer(0);
		status=1;
		starttimer(0,min-time);
		status=0;
...
while(A_buff[base].ackstatus==1) //slide the receiver side window
{
	base++;
}
...
void A_timerinterrupt()
{
	int min,locseq;
	revaltimer(&min,&locseq);
	A_buff[locseq].starttime=time;
	A_buff[locseq].endtime=A_buff[locseq].starttime+TIMEOUT;
	tolayer3(0,A_buff[locseq].pack);
	revaltimer(&min,&locseq);
	starttimer(0,min-time);
	status=0;
	A_transport++;
...
void A_init()
{
	...
	while(i<1000)
	{
		A_buff[i].pack.acknum=-11;
		A_buff[i].pack.seqnum=i;
		A_buff[i].ackstatus=-1;
		A_buff[i].starttime=0.0;
...
struct pkt packet;
{
	...
	{
		if((packet.seqnum==B_buff[rcv_base].bseqnum)&&(B_buff[rcv_base].rcvstatus==-1))
		{
			B_buff[rcv_base].rcvstatus=1;		/*on correct reception of the seqnum as expected*/
			tolayer5(1,	packet.payload);		/*deliver data to application layer*/
			B_application++;					/*increment the Application number counter*/
			/*Enter the expected seqnumber*/
			packet.acknum=B_buff[rcv_base].bseqnum;
			rcv_base++;
			...
			duplicateAck=packet;	//duplicate ACK packet marked. Thus we have maintained to know if the next sequence number will be awaited
		}
		else if(packet.seqnum>B_buff[rcv_base].bseqnum)	//if the received seq is != rcv_base then update table
		{
			B_buff[packet.seqnum].rcvstatus=1;

			packet.acknum=B_buff[packet.seqnum].bseqnum;
		...
		{// B send ack for packet that B already received
			packet.acknum=B_buff[packet.seqnum].bseqnum;
		...
		while(B_buff[rcv_base].rcvstatus==1) //slide the receiver side window
		{
			tolayer5(1,	packet.payload);		/*deliver data to application layer*/
			B_application++;					/*increment the Application number counter*/
			rcv_base++;

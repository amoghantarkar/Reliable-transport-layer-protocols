...
{A_application++;
if(status==1)
{printf("\nmessage printed in A_output %d\n",A_application);
//count of the number of message sent from application layer to transport layer

int size=sizeof(packet.seqnum)+sizeof(packet.acknum)+sizeof(packet.payload);
char buffer[size];					//buffer used to be sent to checksum function

//	copy message data to payload //

packet.seqnum=flag_a;	//printf("message data %s",message.data);
memcpy(packet.payload,message.data,sizeof(message.data));

memcpy(buffer,&packet.seqnum,sizeof(packet.seqnum));
memcpy(buffer+sizeof(packet.seqnum),&packet.acknum,sizeof(packet.acknum));
//printf("packet payload %s",packet.payload);
memcpy(buffer+sizeof(packet.seqnum)+sizeof(packet.acknum),packet.payload,sizeof(packet.payload));
//printf("buffer %s",buffer);

//	calculate the checksum //
packet.checksum=CheckSum((int *)buffer,sizeof(buffer));
printf("\nchecksum is %d\n",packet.checksum);																				//printf("\npacket %d",packet.checksum);
//printf("\nbuffer %s",packet.payload);
//	send to the other side //
if(flag_a==0)								//for next sequence number=1 as seq 0 was sent
{
	tolayer3(0,packet);  					//sent the packet from A (ie 0 ) to 3 layer packet which forwards to B
	status=0;
	starttimer(0,TIMEOUT);					// starttimer called
	//				printf("\nmessage sent to B %d",A_transport);
	A_transport++;							//count of message  sent to network layer from A_transport layer

}
else if(flag_a==1)			//for next sequence number=0 as seq 1 was sent
{
	tolayer3(0,packet);  					//sent the packet from A (ie 0 ) to 3 layer packet which forwards to B
	starttimer(0,TIMEOUT);					// starttimer called
	//printf("\nmessage sent to B %d",A_transport);
...
struct pkt packet;
{
	printf("A_input:%d:%d:%f\n", packet.seqnum, packet.acknum,time);
	int size_B=sizeof(packet.seqnum)+sizeof(packet.acknum)+sizeof(packet.payload);
	char buffer_Bs[size_B];				//buffer used to be sent to checksum function
	int compute;					//compute checksum

	memcpy(buffer_Bs,&packet.seqnum,sizeof(packet.seqnum));
	memcpy(buffer_Bs+sizeof(packet.seqnum),&packet.acknum,sizeof(packet.acknum));
	memcpy(buffer_Bs+sizeof(packet.seqnum)+sizeof(packet.acknum),&packet.payload,sizeof(packet.payload));
	compute=CheckSum((int *)buffer_Bs,sizeof(buffer_Bs));

	printf("\nin A_input\n");

	if(packet.checksum==compute
			&& packet.acknum==flag_a){

		if(flag_a==0)				//if ack=0 was expected
		{
			flag_a=1;
			stoptimer(0);
			status=1;
...
void A_timerinterrupt()
{
	...
	tolayer3(0,packet);
	//sent the packet from A (ie 0 ) to 3 layer packet which forwards to B
	A_transport++;
...
{
	printf("\nA_init initialized\n");
	//struct pkt packet;	//create structure packet
	packet.acknum=0;	//acknum initialized of the packet struct
	packet.seqnum=0;	//seqnum ------------||------------

	//	declarations
	//	int flag_a=0;		//flag to denote if sequence no is 0 or 1; initialized to zero
	TIMEOUT=15.0;	//global timeout initialized
	char buffer[20];	//buffer used to be sent to checksum function
	...
	// initially set the acknum and seqnum to 0 //
...
struct pkt packet;
{
	...
	int size_B=sizeof(packet.seqnum)+sizeof(packet.acknum)+sizeof(packet.payload);
	char buffer_B[size_B],buffer_Bs[size_B];				//buffer used to be sent to checksum function
	int compute;					//compute checksum
	...
	if(packet.checksum==compute)		//if no loss in packet
		if(packet.seqnum==flag_b) {
			if(flag_b==0)
			{
				packet.acknum=flag_b;
				flag_b=1;
	...
	packet.checksum=compute;
	tolayer3(1,packet);		//sending the packet to A ..from B(ie 1) to back to 3 layer
	printf("packet %s\n checksum %d",packet.payload,packet.checksum);
	// send the valid acknowledged data to sender
...
//	declarations
//int flag_a=0;		//flag to denote if sequence no is 0 or 1; initialized to zero
//	TIMEOUT=1000.00;	//global timeout initialized
//char buffer[20];	//buffer used to be sent to checksum function
